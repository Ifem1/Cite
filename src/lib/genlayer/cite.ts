...
async function sendWrite(method: string, args: unknown[]): Promise<TransactionResult> {
  ...
  // GenLayer consensus (proposing → committing → revealing → accepted) takes 1–5 min.
  const receipt = await client.waitForTransactionReceipt({
    hash: hash as `0x${string}` & { length: 66 },
    timeout: 360_000,   // 6 minutes
    interval: 4_000,    // poll every 4 s
  } as Parameters<typeof client.waitForTransactionReceipt>[0])

  // txExecutionResult: 0 = pending/success, 1 = success (varies by version), negative = failed
  // If we got a receipt without throwing, treat as confirmed.
  const execResult = (receipt as { txExecutionResult?: unknown }).txExecutionResult
  const failed = typeof execResult === 'number' && execResult < 0
